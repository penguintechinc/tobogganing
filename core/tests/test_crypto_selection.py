...
from core.crypto.keys import InAppKeyProvider, AwsKmsKeyProvider, GcpKmsKeyProvider
...
def test_aws_provider_flag_off_returns_inapp(
    ...
    with patch.dict("os.environ", {"KMS_PROVIDER": "aws"}):
        with patch("core.crypto.selection.feature_enabled", return_value=False):
...
def test_aws_provider_unlicensed_returns_inapp(
    ...
    with patch.dict("os.environ", {"KMS_PROVIDER": "aws"}):
        with patch("core.crypto.selection.feature_enabled", return_value=True):
            with patch(
                "core.crypto.selection._is_licensed_for_tier", return_value=False
            ):
                with caplog.at_level(logging.WARNING):
                    provider = build_signing_provider(registry)
                assert isinstance(provider, InAppKeyProvider)
...
def test_aws_provider_licensed_returns_aws_provider(
    ...
    with patch.dict(
        ...
        with patch("core.crypto.selection.feature_enabled", return_value=True):
            with patch(
                "core.crypto.selection._is_licensed_for_tier", return_value=True
            ):
...
def test_registry_entitlement_for_core_external_kms(
    ...
    """Test (g): entitlement_for('core.external_kms').tier == 'enterprise' after registration."""
    registry.register_entitlements(
        [Entitlement(feature="core.external_kms", tier="enterprise")]
    )
    entitlement = registry.entitlement_for("core.external_kms")
