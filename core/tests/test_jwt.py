...
class TestJwtEncoding:
    ...
    @pytest.mark.asyncio
    async def test_encode_access_token_success(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_encode_access_token_missing_required_claim(self) -> None:
        ...
        with pytest.raises(ValueError, match="Missing required claims"):
            await encode_access_token(claims, provider)

    @pytest.mark.asyncio
    async def test_encode_includes_iat_and_exp(self) -> None:
...
class TestJwtDecoding:
    ...
    @pytest.mark.asyncio
    async def test_decode_token_success(self) -> None:
    ...
    def test_decode_token_missing_tenant_returns_none(self) -> None:
        ...
        # Create token manually without tenant claim using pyjwt
        claims = {
            "sub": "user123",
            "iss": "test-app",
            "aud": "test-app",
    ...
    @pytest.mark.asyncio
    async def test_decode_token_tampered_returns_none(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_decode_token_invalid_signature_returns_none(self) -> None:
        ...
        token = await encode_access_token(claims, provider1, ttl_hours=1)
    ...
    @pytest.mark.asyncio
    async def test_decode_token_kid_in_header(self) -> None:
...
class TestJwtRoundTrip:
    ...
    @pytest.mark.asyncio
    async def test_round_trip_preserves_all_claims(self) -> None:
        ...
        token = await encode_access_token(original_claims, provider, ttl_hours=1)
        decoded = decode_token(token, provider)
