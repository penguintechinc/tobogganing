...
class TestTestManager:
    """Test TestManager operations."""

    @pytest.mark.asyncio
    async def test_create_test(self, mock_db: MagicMock, now: datetime) -> None:
        """Test creating a new performance test."""
        manager = TestManager(mock_db, "tenant1")
        ...
        mock_db.perf_test_results.create = MagicMock(return_value=test_obj)
    ...
    @pytest.mark.asyncio
    async def test_get_test(self, mock_db: MagicMock, now: datetime) -> None:
        """Test getting a test result by ID."""
        manager = TestManager(mock_db, "tenant1")
        ...
        mock_db.perf_test_results.select = MagicMock(return_value=test_obj)

        result = await manager.get_test("test-1")

        assert result is not None
        assert result.id == "test-1"
        assert result.latency_ms == 45.2
        assert result.throughput == 100.5
        mock_db.perf_test_results.select.assert_called_once()

    @pytest.mark.asyncio
    async def test_get_test_not_found(self, mock_db: MagicMock) -> None:
        """Test getting a non-existent test returns None."""
        manager = TestManager(mock_db, "tenant1")

        mock_db.perf_test_results.select = MagicMock(return_value=None)

        result = await manager.get_test("nonexistent")

        assert result is None

    @pytest.mark.asyncio
    async def test_record_result(self, mock_db: MagicMock, now: datetime) -> None:
        """Test recording test result details."""
        manager = TestManager(mock_db, "tenant1")
        ...
        mock_db.perf_test_results.select = MagicMock(return_value=test_obj)
        mock_db.perf_test_results.update = MagicMock()
        ...
        assert result.latency_ms == 45.2
        mock_db.perf_test_results.update.assert_called_once()

    @pytest.mark.asyncio
    async def test_list_results_all(self, mock_db: MagicMock, now: datetime) -> None:
        """Test listing all test results for tenant."""
        manager = TestManager(mock_db, "tenant1")
        ...
        mock_db.perf_test_results.select_list = MagicMock(return_value=[test1, test2])

        results = await manager.list_results(limit=100, offset=0)
    ...
    @pytest.mark.asyncio
    async def test_list_results_filtered_by_device(
        self, mock_db: MagicMock, now: datetime
    ) -> None:
        """Test listing test results filtered by device."""
        manager = TestManager(mock_db, "tenant1")
        ...
        results = await manager.list_results(device_id="device-1")
    ...
    @pytest.mark.asyncio
    async def test_list_results_filtered_by_type(
        self, mock_db: MagicMock, now: datetime
    ) -> None:
        """Test listing test results filtered by type."""
        manager = TestManager(mock_db, "tenant1")
        ...
        results = await manager.list_results(test_type="latency")
    ...
    @pytest.mark.asyncio
    async def test_list_results_filtered_by_status(
        self, mock_db: MagicMock, now: datetime
    ) -> None:
        """Test listing test results filtered by status."""
        manager = TestManager(mock_db, "tenant1")
        ...
        results = await manager.list_results(status="pending")

        assert len(results) == 1
        assert results[0].status == "pending"

    @pytest.mark.asyncio
    async def test_list_results_empty(self, mock_db: MagicMock) -> None:
        """Test listing results returns empty list when no results."""
        manager = TestManager(mock_db, "tenant1")

        mock_db.perf_test_results.select_list = MagicMock(return_value=[])

        results = await manager.list_results()

        assert len(results) == 0

    @pytest.mark.asyncio
    async def test_list_tests(self, mock_db: MagicMock, now: datetime) -> None:
        ...
        mock_db.perf_test_results.select_list = MagicMock(return_value=[test1])

        results = await manager.list_tests()

        assert len(results) == 1
        assert results[0].id == "test-1"

    @pytest.mark.asyncio
    async def test_tenant_scoping(self, mock_db: MagicMock, now: datetime) -> None:
        """Test that all queries are tenant-scoped."""
        manager = TestManager(mock_db, "tenant-x")

        mock_db.perf_test_results.select = MagicMock(return_value=None)

        await manager.get_test("test-1")

        # Verify tenant was passed to select
        call_kwargs = mock_db.perf_test_results.select.call_args[1]
        assert call_kwargs["tenant"] == "tenant-x"

    @pytest.mark.asyncio
    async def test_delete_test(self, mock_db: MagicMock, now: datetime) -> None:
        ...
        mock_db.perf_test_results.select = MagicMock(return_value=test_obj)
        mock_db.perf_test_results.delete = MagicMock()

        result = await manager.delete_test("test-1")

        assert result is True
        mock_db.perf_test_results.delete.assert_called_once()

    @pytest.mark.asyncio
    async def test_delete_test_not_found(self, mock_db: MagicMock) -> None:
        """Test deleting a non-existent test returns False."""
        manager = TestManager(mock_db, "tenant1")

        mock_db.perf_test_results.select = MagicMock(return_value=None)

        result = await manager.delete_test("nonexistent")
