...
@celery_app.task(name="hub_api.modules.waddleperf_cluster.worker.tasks.run_server_test")
def run_server_test(
    job_id: str,
    tenant: str,
    module: str,
...
async def _alert_sweep_async(db: Any | None = None) -> int:
    ...
    try:
        from hub_api.modules.waddleperf_cluster.services.alert_evaluator import (
            AlertEvaluator,
        )
...
@celery_app.task(name="hub_api.modules.waddleperf_cluster.worker.tasks.alert_sweep")
...
@celery_app.task(name="hub_api.modules.waddleperf_cluster.worker.tasks.autoperf_cycle")
def autoperf_cycle(
    job_id: str,
    tenant: str,
    module: str,
