...
11. **[SECURITY-REQUIRED · production-gating] Headend service-auth redesign (finding C).** `firewall/rules`, `wireguard/peers`, `headend/ports` are guarded only by a global static `HEADEND_API_TOKEN` — bypassing the tenant/scope OIDC model every other route uses — and are not scoped to the caller's tenant (one shared secret → bulk cross-tenant read of all users' rules and all peers). Replace with a **per-cluster machine-JWT** (tenant+scope claims; the Go headend authenticates as its cluster) and scope every query to the authenticated headend's tenant. Also reconciles the token split (Go uses `authToken` for firewall/ports, `CLUSTER_API_KEY` for peers). Needs coordinated Go-headend change → P-B (auth unification). **MUST land before the Quart brain serves production traffic.**
12. **[SECURITY-REQUIRED · production-gating] Refresh-token rotation/replay (finding D).** `POST /api/v1/auth/{token,refresh}` issue non-rotating 24h refresh tokens with no `jti`/revocation store and no subject re-validation at refresh → a captured refresh token is replayable, and a revoked cluster's refresh token keeps working. Add rotating single-use refresh tokens + a `jti`/revocation store + subject re-check at refresh (the client must store the rotated token → Go coordination). P-B. **MUST land before production.**

> Findings A (identity binding on `auth/token`'s cluster branch) and B (client-supplied `?tenant=` on `headend/ports`) were self-contained and are fixed on `feature/pa-license-gate` (see Task 3 fix commit); C and D above are the ported-legacy auth-model issues that need the coordinated redesign.
