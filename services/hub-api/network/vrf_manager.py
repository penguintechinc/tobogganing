"""
VRF (Virtual Routing and Forwarding) Management for Tobogganing
...
class VRFConfiguration:
    ...
    name: str
    description: str
    # Route Distinguisher (ASN:value or IP:value)
    rd: str
    # Route Targets for import
    rt_import: List[str] = field(default_factory=list)
    # Route Targets for export
    rt_export: List[str] = field(default_factory=list)
    # CIDR blocks assigned to this VRF
...
class VRFManager:
    ...
    def _init_database(self):
        ...
        cursor.execute("""
            CREATE INDEX IF NOT EXISTS idx_vrfs_status
            ON vrfs(status, is_active)
        """)

        cursor.execute("""
            CREATE INDEX IF NOT EXISTS idx_ospf_areas_vrf
            ON ospf_areas(vrf_id)
        """)
        ...
        try:
            ...
            cursor.execute(
                ...
                UPDATE vrfs SET
                    name = ?, description = ?, rd = ?,
                    rt_import = ?, rt_export = ?,
                    ip_ranges = ?, status = ?, updated_at = ?,
                    is_active = ?,
                    ospf_enabled = ?, ospf_router_id = ?,
                ...
                (
                    ...
                    json.dumps(vrf.ospf_areas),
                    json.dumps(vrf.ospf_networks),
                    vrf.id
                ))
    ...
    async def delete_vrf(self, vrf_id: str) -> bool:
        ...
        try:
            ...
            cursor.execute(
                "DELETE FROM ospf_areas WHERE vrf_id = ?",
                (vrf_id,))
            cursor.execute(
                "DELETE FROM ospf_neighbors WHERE vrf_id = ?",
                (vrf_id,))
    ...
    async def get_vrf(self, vrf_id: str) -> Optional[VRFConfiguration]:
        ...
        try:
            ...
            cursor.execute(
                """
                SELECT id, name, description, rd, rt_import,
                       rt_export, ip_ranges,
    ...
    async def list_vrfs(
            self,
            active_only: bool = True) -> List[VRFConfiguration]:
    ...
    async def get_ospf_neighbors(self, vrf_id: str) -> List[OSPFNeighbor]:
        ...
        try:
            ...
            cursor.execute(
                """
                SELECT neighbor_id, vrf_id, neighbor_ip, interface,
                       area_id, state, priority, dead_interval,
                       hello_interval, last_seen
        ...
        try:
            ...
            if vrf.ospf_enabled and vrf.ospf_router_id:
                ...
                for network in vrf.ospf_networks:
                    # nosec B104 - OSPF area ID, not a bind address
                    area_id = network.get('area', '0.0.0.0')
                    net = network.get('network')
                    if net:
                        config_lines.append(
                            f" network {net} area {area_id}")
            ...
            logger.info(
                "Generated FRR VRF configuration",
                vrf_id=vrf.id,
            ...
            logger.error(
                "Failed to apply VRF configuration",
                vrf_id=vrf.id,
                error=str(e))
    ...
    async def _remove_vrf_config(self, vrf_id: str):
        ...
        try:
            ...
            logger.info(
                "Removed VRF configuration from FRR",
                vrf_id=vrf_id)

        except Exception as e:
            logger.error(
                "Failed to remove VRF configuration",
                vrf_id=vrf_id,
                error=str(e))
    ...
    async def generate_frr_config(self, vrf_id: str) -> str:
        ...
        try:
            ...
            config_lines = [
                ...
                "! Generated by Tobogganing Manager",
