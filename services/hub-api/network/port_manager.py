...
class HeadendPortConfig:
    ...
    def get_tcp_range_string(self) -> str:
        """Get TCP ranges as comma-separated string.

        Example: '8000-8100,9000,9500-9600'
        """
        ranges = []
    ...
    def get_udp_range_string(self) -> str:
        """Get UDP ranges as comma-separated string.
...
class PortConfigManager:
    ...
    async def get_headend_config(
            self, headend_id: str) -> Optional[HeadendPortConfig]:
    ...
    async def get_cluster_config(
            self,
            cluster_id: str) -> Dict[str, HeadendPortConfig]:
    ...
    async def add_port_range(
        ...
        if await self._has_port_overlap(headend_id, port_range):
            raise ValueError(
                f"Port range "
                f"{port_range.start_port}-{port_range.end_port} "
        ...
        def _add_range():
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("""
                    ...
                    (id, headend_id, cluster_id, start_port, end_port,
                     protocol, description, enabled, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    port_range.id,
                    headend_id,
        ...
        logger.info(
            f"Added port range "
            f"{port_range.start_port}-{port_range.end_port} "
            f"({port_range.protocol.value}) for headend {headend_id}")
    ...
    async def remove_port_range(self, range_id: str) -> bool:
        ...
        def _remove_range():
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "DELETE FROM port_ranges WHERE id = ?",
                    (range_id,))
    ...
    async def update_port_range(
        ...
        def _update_range():
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                # nosec B608 - set_clause from hardcoded column names
                cursor.execute(
                    f"UPDATE port_ranges SET {set_clause} "
                    f"WHERE id = ?",
                    values)
                return cursor.rowcount > 0
        ...
        def _check_overlap():
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute(
                    ...
                    WHERE headend_id = ? AND protocol = ? AND
                          enabled = 1
                    AND (
    ...
    async def set_default_config(
            self, headend_id: str, cluster_id: str) -> None:
        """Set default port configuration for a headend."""
        # Default proxy ports
        default_ranges = [
            PortRange(
                start_port=8443, end_port=8443,
                protocol=PortProtocol.TCP,
                description="HTTPS Proxy"),
            PortRange(
                start_port=8444, end_port=8444,
                protocol=PortProtocol.TCP,
                description="TCP Proxy"),
            PortRange(
                start_port=8445, end_port=8445,
        ...
        for port_range in default_ranges:
            try:
                ...
                logger.warning(
                    f"Could not add default range "
                    f"{port_range.start_port}-{port_range.end_port}: {e}")
