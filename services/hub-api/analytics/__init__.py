...
Schema for analytics tables (client_analytics, headend_analytics, traffic_stats)
is managed by Alembic migrations (see database/migrations/).
This module performs runtime queries only via penguin-dal (AsyncDB).
"""
...
class AnalyticsManager:
    ...
    def record_client_activity(self, client_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            existing = db(
                db.client_analytics.client_id == client_data['client_id']
            ).select().first()
            ...
            if existing:
                db(
                    ...
                    connected_headend=client_data.get(
                        'connected_headend', existing.connected_headend
                    ),
                    connection_duration=client_data.get(
                        'connection_duration', existing.connection_duration
                    ),
                    bytes_sent=client_data.get('bytes_sent', existing.bytes_sent),
                    bytes_received=client_data.get(
                        'bytes_received', existing.bytes_received
                    ),
                    packets_sent=client_data.get('packets_sent', existing.packets_sent),
                    packets_received=client_data.get(
                        'packets_received', existing.packets_received
                    ),
    ...
    def record_headend_stats(self, headend_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            existing = db(
                db.headend_analytics.headend_id == headend_data['headend_id']
            ).select().first()
            ...
            if existing:
                db(
                    ...
                    active_connections=headend_data.get(
                        'active_connections', existing.active_connections
                    ),
                    total_connections=headend_data.get(
                        'total_connections', existing.total_connections
                    ),
                    bytes_proxied=headend_data.get('bytes_proxied', existing.bytes_proxied),
                    packets_proxied=headend_data.get(
                        'packets_proxied', existing.packets_proxied
                    ),
                    cpu_usage_percent=headend_data.get(
                        'cpu_usage_percent', existing.cpu_usage_percent
                    ),
                    memory_usage_mb=headend_data.get(
                        'memory_usage_mb', existing.memory_usage_mb
                    ),
                    disk_usage_percent=headend_data.get(
                        'disk_usage_percent', existing.disk_usage_percent
                    ),
                    network_errors=headend_data.get(
                        'network_errors', existing.network_errors
                    ),
                    auth_successes=headend_data.get(
                        'auth_successes', existing.auth_successes
                    ),
    ...
    def get_os_statistics(self, days_back: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            # Raw SQL via SQLAlchemy engine for aggregation queries
            ...
            os_query = sa_text("""
                ...
                WHERE last_seen >= :cutoff AND os_name IS NOT NULL
            ...
            with db.engine.connect() as conn:
                result = conn.execute(os_query, {'cutoff': cutoff_date})
                for row in result:
                    os_name, os_version, arch, count, _avg, _bytes = row
            ...
            # Recent activity counts via penguin-dal
            last_24h = datetime.utcnow() - timedelta(hours=24)
            last_hour = datetime.utcnow() - timedelta(hours=1)

            os_stats['active_last_24h'] = db(
                db.client_analytics.last_seen >= last_24h
            ).count()
            os_stats['active_last_hour'] = db(
                db.client_analytics.last_seen >= last_hour
            ).count()

            # Convert defaultdicts to regular dicts for JSON serialization
            os_stats['by_os'] = dict(os_stats['by_os'])
            for os_name in os_stats['by_os']:
                os_stats['by_os'][os_name]['versions'] = dict(
                    os_stats['by_os'][os_name]['versions']
                )
                os_stats['by_os'][os_name]['architectures'] = dict(
                    os_stats['by_os'][os_name]['architectures']
                )
    ...
    def get_traffic_statistics(self, days_back: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            headend_rows = db(
                db.headend_analytics.last_heartbeat >= cutoff_date
            ).select()
            ...
            for headend in headend_rows:
                ...
                traffic_data['by_headend'][headend.headend_id] = {
                    ...
                    'last_heartbeat': (
                        headend.last_heartbeat.isoformat()
                        if headend.last_heartbeat
                        else None
                    ),
                    'auth_success_rate': (
                        (
                            headend.auth_successes
                            / (headend.auth_successes + headend.auth_failures + 1)
                        )
                        * 100
                        if (headend.auth_successes or headend.auth_failures)
                        else 0
                    ),
                }

                if headend.region:
                    traffic_data['by_region'][headend.region]['bytes'] += (
                        headend.bytes_proxied or 0
                    )
                    traffic_data['by_region'][headend.region]['packets'] += (
                        headend.packets_proxied or 0
                    )
                    traffic_data['by_region'][headend.region]['connections'] += (
                        headend.total_connections or 0
                    )

            traffic_data['by_region'] = dict(traffic_data['by_region'])

            # Timeline from traffic_stats table
            timeline_rows = db(
                (db.traffic_stats.stat_type == 'hourly')
                & (db.traffic_stats.timestamp >= cutoff_date)
            ).select(orderby=db.traffic_stats.timestamp)

            for stat in timeline_rows:
    ...
    def search_agents_and_headends(
        ...
        try:
            ...
            if filter_type in ['all', 'agents']:
                if search_term:
                    # Use SQLAlchemy LIKE via FieldProxy .like()
                    pattern = f"%{search_term}%"
                    from penguin_dal.query import Query
                    from sqlalchemy import or_

                    ca = db.client_analytics
                    agent_filter = Query(
                        or_(
                            ca.client_id.column.like(pattern),
                            ca.hostname.column.like(pattern),
                            ca.os_name.column.like(pattern),
                            ca.ip_address.column.like(pattern),
                            ca.os_version.column.like(pattern),
                        ),
                        table=ca.table,
                    )
                    agents = db(agent_filter).select(
                        limitby=(0, limit),
                        orderby=(
                            db.client_analytics.hostname
                            if sort_by == 'hostname'
                            else (
                                db.client_analytics.os_name
                                if sort_by == 'os_name'
                                else db.client_analytics.last_seen
                            )
                        ),
                    )
                    results['total_agents'] = db(agent_filter).count()
                else:
                    orderby = (
                        db.client_analytics.hostname
                        if sort_by == 'hostname'
                    ...
                    all_filter = Query(
                        sa_true(), table=db.client_analytics.table
                    )
                    agents = db(all_filter).select(
                        limitby=(0, limit), orderby=orderby
                    )
                    results['total_agents'] = db(all_filter).count()
                ...
                for agent in agents:
                    ...
                    results['agents'].append({
                        ...
                        'status': status,
                        'total_bytes': (agent.bytes_sent or 0) + (
                            agent.bytes_received or 0
                        ),
                    })
            ...
            if filter_type in ['all', 'headends']:
                if search_term:
                    ...
                    ha = db.headend_analytics
                    headend_filter = Query(
                        or_(
                            ha.headend_id.column.like(pattern),
                            ha.hostname.column.like(pattern),
                            ha.region.column.like(pattern),
                            ha.cluster_id.column.like(pattern),
                        ),
                        table=ha.table,
                    )
                    headends = db(headend_filter).select(
                        limitby=(0, limit),
                        orderby=(
                            db.headend_analytics.hostname
                            if sort_by == 'hostname'
                            else db.headend_analytics.last_heartbeat
                        ),
                    )
                    results['total_headends'] = db(headend_filter).count()
                else:
                    from penguin_dal.query import Query
                    from sqlalchemy import true as sa_true

                    all_filter = Query(
                        sa_true(), table=db.headend_analytics.table
                    )
                    orderby = (
                        db.headend_analytics.hostname
                        if sort_by == 'hostname'
                    ...
                    headends = db(all_filter).select(
                        limitby=(0, limit), orderby=orderby
                    )
                    results['total_headends'] = db(all_filter).count()

                for headend in headends:
                    last_heartbeat_minutes = (
                        datetime.utcnow() - headend.last_heartbeat
                    ).total_seconds() / 60
            ...
            return {
                'agents': [], 'headends': [], 'total_agents': 0, 'total_headends': 0
            }
