...
verify() {
    ...
    # In a LINKED WORKTREE, .git is a file pointing at the real gitdir, so
    # "$root/.git/hooks" does not exist and every hook reads as NOT INSTALLED.
    # rev-parse --git-path resolves the true hooks dir in both layouts, and
    # honours core.hooksPath. A false negative here is the exact failure this
    # check exists to catch, so it must not depend on the repo layout.
